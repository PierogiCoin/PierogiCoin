...
export default function HowItWorks() {
  ...
  return (
    <section id="jak-pracujemy" ref={containerRef} className="relative py-24 bg-white dark:bg-black overflow-hidden">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        <div className="absolute left-1/2 top-32 bottom-0 w-0.5 bg-slate-300 dark:bg-gray-800 hidden md:block -translate-x-1/2">
          ...
          {STEPS.map((step, index) => (
            <div key={step.id} className={`step-card flex flex-col md:flex-row items-center gap-8 ${index % 2 !== 0 ? 'md:flex-row-reverse' : ''}`}>
              ...
              <div className="relative flex-shrink-0 z-10">
                ...
                <div className="absolute -top-3 -right-3 w-8 h-8 bg-slate-200 dark:bg-gray-900 rounded-full border border-slate-300 dark:border-gray-700 flex items-center justify-center text-sm font-bold text-slate-900 dark:text-white">
                  {step.id}
                </div>
              </div>
              ...
              <div className={`flex-1 text-center md:text-left ${index % 2 !== 0 ? 'md:text-right' : ''}`}>
                ...
                {/* Benefit Box */}
                <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 text-sm ${step.color}`}>
